...
describe('BeaverList Component', () => {
    ...
    it('renders a list of beavers with name, age and health', () => {
        const mockBeavers = [
            { name: 'Justin', age: 20, health: 80, birthday: 0 },
            { name: 'Bieber', age: 50, health: 100, birthday: 0 },
        ];
        ...
        expect(screen.getByText(`Age: 20 days`)).toBeInTheDocument();
        expect(screen.getByText('80%')).toBeInTheDocument();

        expect(screen.getByText('Bieber')).toBeInTheDocument();
        expect(screen.getByText(`Age: 50 days`)).toBeInTheDocument();
